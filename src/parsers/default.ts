...
namespace MapperLib {
  export interface ParserSettings {
    /**
     * Should array mapping be allowed
     */
    arrayMap?: boolean;
  }
  ...
  export class MapAttributeValueParser {
    ...
    protected getElementValueOrDataValueAttribute(
      ...
      if (
        mapperConfig.dataValueAttributeToUseForGet &&
    ...
    protected setElementValueOrDataValueAttribute(
      mapperConfig: MapperConfiguration,
      el: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement,
      valueToSet: string
    ) {
      if (
        mapperConfig.dataValueAttributeToUseForSet &&
    ...
    ): MapAttributeValueGetResult {
      const value = this.getElementValueOrDataValueAttribute(
        mapperConfig,
        element
      );
    ...
    private parseHtmlSelectValue(
      ...
      const selectedValues = Array.from(element.options)
        ...
        .map((x) => {
          ...
          if (value === null || value === undefined) value = x.text;
    ...
    private parseHtmlInputValue(
      ...
      let returnValue = this.getElementValueOrDataValueAttribute(
        mapperConfig,
        element
      ) as any;
      ...
      switch (element.type) {
        ...
        case "radio":
          ...
          const elements = Array.from(
            containerElement.querySelectorAll<HTMLInputElement>(querySelector)
          );

          const elementsChecked = Array.from(elements).filter(
            (x) => x.checked === true
          );
          ...
          if (element.type === "checkbox") {
            ...
            const haveValueAttributes =
              elements.filter(
                (x) =>
                  (x.hasAttribute("value") && x.getAttribute("value")) ||
                  x.hasAttribute(mapperConfig.dataValueAttributeToUseForGet)
              ).length > 0;
            ...
            else arrayToParse = arrayToParse.concat(elements);
      ...
      switch (element.type) {
        ...
        case "radio":
          ...
          if (Array.isArray(valueToSet)) {
            elements.forEach((x) => {
              let elementValue: string | boolean =
                this.getElementValueOrDataValueAttribute(mapperConfig, x);
        ...
        default:
          this.setElementValueOrDataValueAttribute(
            mapperConfig,
            element,
    ...
    protected getElementsByMapAttribute(
      containerElement: HTMLElement,
      mapAttribute: string
    ) {
    ...
    public getValue(
      ...
      if (mapElement instanceof HTMLInputElement) {
        result = this.parseHtmlInputValue(
          mapperConfig,
          containerElement,
          mapElement
        );
    ...
    public setValue(
      ...
      if (mapElement instanceof HTMLInputElement) {
        ...
        if (!Array.isArray(valueToSet)) valueToSet = [valueToSet];
        ...
        Array.from(mapElement.options).forEach((opt) => {
          opt.selected =
            valueToSet.indexOf(
              this.getElementValueOrDataValueAttribute(mapperConfig, opt)
            ) >= 0;
        });
      } else {
