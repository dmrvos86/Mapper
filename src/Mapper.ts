...
class Mapper {
   ...
   */
  public static elementValueParsers: {
  ...
  public static elementValueParsersSettings: {
    [key: string]: MapperLib.ParserSettings;
  } = {
    default: {},
    json: MapperLib.MapAttributeJsonParser.JsonParserSettings,
  };
   ...
   * @param mapAttribute
   */
  protected getFirstElementByMapAttribute(
  ...
  private getValueByMapAttribute(
    ...
    const mapElement = this.getFirstElementByMapAttribute(
      containerElement,
      mapAttribute
    );
    const parser = this.getElementParser(mapElement);
    return Mapper.elementValueParsers[parser].getValue(
      this.configuration,
      mapElement,
      containerElement
    ).value;
  ...
  private setValueByMapAttribute(
    ...
    const parser = this.getElementParser(mapElement);
    Mapper.elementValueParsers[parser].setValue(
      this.configuration,
      mapElement,
      containerElement,
      valueToSet
  ...
  private preProcess(): void {
    ...
    this.parseElements(this.containerElement)
      ...
      .filter(
        (x) =>
          x.getAttribute("map").indexOf("[]") < x.getAttribute("map").length - 2
      )
      .forEach((x) => {
    ...
    const mapOriginalElements = Array.from(
      this.containerElement.querySelectorAll("[map-original]")
    );

    // distinct map-original attributes
    let mapOriginalAttributes = mapOriginalElements.map((x) =>
      x.getAttribute("map-original")
    );
    ...
    mapOriginalAttributes.forEach((map) => {
      ...
      const elements = mapOriginalElements.filter(
        (x) => x.getAttribute("map-original") === map
      );
  ...
  public getData() {
    ...
    const mappedObject = {};
    const steps = this.buildMapProcedureStepsForAllElements(
    ...
    const scriptFunctions = {
      PROPERTY_TRAVERSE: (
        mapAttribute: string,
        step: MapperLib.MapStep,
        lastCreatedObject: any
      ) => {
        if (step.isLastStep) {
          //if element value should be mapped
          var elementValue = this.getValueByMapAttribute(
            this.containerElement,
            mapAttribute
          );
          ...
          lastCreatedObject[step.propertyName] =
            lastCreatedObject[step.propertyName] || step.defaultPropertyValue;
          return lastCreatedObject[step.propertyName];
        }
      },
      ARRAY_ITEM: (
        _mapAttribute: string,
        step: MapperLib.MapStep,
        lastCreatedObject: any[]
      ) => {
        const isKeyValueSearch =
          step.matchKey !== undefined && step.matchValue !== undefined;
        let isIndexSearch = step.matchIndex >= 0; //x[0].y

        // key-value search
        if (isKeyValueSearch) {
          const filteredArray = lastCreatedObject.filter(
            (x) => x[step.matchKey] == step.matchValue
          );
        ...
        // index search
          ...
          lastCreatedObject[step.matchIndex] =
            lastCreatedObject[step.matchIndex] || step.defaultPropertyValue;
  ...
  public setData(dataToMap: {}) {
    if (!(dataToMap instanceof Object)) return;
    ...
    const scriptFunctions = {
      PROPERTY_TRAVERSE: (
        ...
        if (step.isLastStep)
          this.setValueByMapAttribute(
            this.containerElement,
            mapAttribute,
            currentPath[step.propertyName]
          );
        ...
        // if path segment is missing - skip it
          ...
          currentPath[step.propertyName] === null ||
          currentPath[step.propertyName] === undefined
        )
          return;
        else throw "Mapper - Invalid mapping on: " + mapAttribute;
      },
      ARRAY_ITEM: (
        mapAttribute: string,
        step: MapperLib.MapStep,
  ...
  private getMapAttributesByElement(nodes: NamedNodeMap) {
    return Array.from(nodes).filter((v) => v.name == "map");
  }
  ...
  private parseElements(containerElement: HTMLElement): HTMLElement[] {
    const elementsWithMapAttribute = Array.from(
  ...
  private buildMapProcedureStepsForAllElements(
    containerElement: HTMLElement
  ): MapperLib.MapAttributeSteps[] {
    const allMapAttributes = this.parseElements(containerElement).map((x) => {
      return {
        map: x.getAttribute("map"),
        parser: this.getElementParser(x),
      };
    });
    ...
    return uniqueMapAttributes.map((x) => {
      return MapperLib.MapProcedureBuilder.buildMapProcedureSteps(
        x.map,
        Mapper.elementValueParsersSettings[x.parser]
      );
    });
  }
   ...
   */
  public static initializeMapperByElementsName(
    containerElement: HTMLElement
  ): Mapper {
    // find all elements without map attribute and with defined name attribute
    const elementsToCheck = Array.from(
      containerElement.querySelectorAll("input,select,textarea")
    ).filter((element) => {
      const attributes = Array.from(element.attributes);
      const mapAttributeCount = attributes.filter((attribute) =>
        attribute.name.startsWith("map")
      ).length;
      ...
      const hasNameAttribute =
        attributes
          ...
          .filter((x) => x.value.length > 0).length > 0; // name attribute defined
    ...
    elementsToCheck.forEach((element) => {
      ...
      // map it as array
      const sameNameCount = elementsToCheck.filter(
        (el) => el.getAttribute("name") === elementName
      ).length;

      if (sameNameCount > 1) mapName += "[]";
   ...
   */
  public static AddMapper(
    name: string,
    valueParser: MapperLib.MapAttributeValueParser
  ) {
