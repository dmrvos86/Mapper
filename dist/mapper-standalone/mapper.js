...
(function (MapperLib) {
    ...
    class MapAttributeValueParser {
        ...
        getElementValueOrDataValueAttribute(mapperConfig, el) {
            if (mapperConfig.dataValueAttributeToUseForGet &&
                el.hasAttribute(mapperConfig.dataValueAttributeToUseForGet))
        ...
        setElementValueOrDataValueAttribute(mapperConfig, el, valueToSet) {
            if (mapperConfig.dataValueAttributeToUseForSet &&
                el.hasAttribute(mapperConfig.dataValueAttributeToUseForSet))
        ...
        parseHtmlInputValue(mapperConfig, containerElement, element) {
            ...
            switch (element.type) {
                ...
                case "radio":
                    ...
                    const elementsChecked = Array.from(elements).filter((x) => x.checked === true);
                    ...
                    if (element.type === "checkbox") {
                        const haveValueAttributes = elements.filter((x) => (x.hasAttribute("value") && x.getAttribute("value")) ||
                            x.hasAttribute(mapperConfig.dataValueAttributeToUseForGet)).length > 0;
                        let arrayToParse = [];
        ...
        setValue(mapperConfig, mapElement, containerElement, valueToSet) {
            ...
            if (mapElement instanceof HTMLInputElement) {
                ...
                Array.from(mapElement.options).forEach((opt) => {
                    opt.selected =
                        valueToSet.indexOf(this.getElementValueOrDataValueAttribute(mapperConfig, opt)) >= 0;
                });
            }
            else {
    ...
    MapAttributeJsonParser.JsonParserSettings = { arrayMap: true };
    ...
    class SegmentInfo {
        ...
        validate() {
            const invalidMapping = (this.mapType === "ARRAY" &&
    ...
    class MapProcedureBuilder {
        ...
        static buildMapProcedureSteps(mapProperty, settings) {
                ...
                switch (segmentInfo.mapType) {
                    case "PROPERTY":
                        const stepData = {
                            ...
                            mapAsArray: (_a = settings.arrayMap) !== null && _a !== void 0 ? _a : segmentInfo.mapAsArray,
...
class Mapper {
    ...
    getData() {
        ...
        steps.forEach((script) => {
            ...
            script.steps.forEach((step) => {
                lastCreatedObject = [
                    scriptFunctions[step.type](script.mapAttribute, step, lastCreatedObject[0]),
                ];
            });
        });
        return mappedObject;
    ...
    setData(dataToMap) {
        ...
        const scriptFunctions = {
            PROPERTY_TRAVERSE: (mapAttribute, step, currentPath) => {
                ...
                else if (currentPath[step.propertyName] === null ||
        ...
        steps.forEach((script) => {
            ...
            script.steps.forEach((step) => {
                if (currentPathSegment[0] !== undefined)
                    currentPathSegment = [
                        scriptFunctions[step.type](script.mapAttribute, step, currentPathSegment[0]),
                    ];
            });
        });
    }
    ...
    parseElements(containerElement) {
        const elementsWithMapAttribute = Array.from(containerElement.querySelectorAll("input,textarea,select")).filter((x) => this.getMapAttributesByElement(x.attributes).length > 0);
    ...
    buildMapProcedureStepsForAllElements(containerElement) {
        ...
        return uniqueMapAttributes.map((x) => {
            return MapperLib.MapProcedureBuilder.buildMapProcedureSteps(x.map, Mapper.elementValueParsersSettings[x.parser]);
        });
    }
    static initializeMapperByElementsName(containerElement) {
        const elementsToCheck = Array.from(containerElement.querySelectorAll("input,select,textarea")).filter((element) => {
            const attributes = Array.from(element.attributes);
            const mapAttributeCount = attributes.filter((attribute) => attribute.name.startsWith("map")).length;
            const hasMapAttribute = mapAttributeCount > 0;
            const hasNameAttribute = attributes
                .filter((x) => x.name === "name")
                .filter((x) => x.value.length > 0).length > 0;
            return !hasMapAttribute && hasNameAttribute;
        });
        elementsToCheck.forEach((element) => {
            const elementName = element.getAttribute("name");
            let mapName = elementName[0].toLowerCase() + elementName.slice(1);
            const sameNameCount = elementsToCheck.filter((el) => el.getAttribute("name") === elementName).length;
            if (sameNameCount > 1)
                mapName += "[]";
...
Mapper.elementValueParsersSettings = {
    default: {},
